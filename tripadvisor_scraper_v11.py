...
URL_FILE = "tripadvisor_list_restaurant.txt"
JSON_FILE = "tripadvisor_scraper_results.json"
CHECKPOINT_FILE = "tripadvisor_checkpoint.json"

USER_DATA_DIR = os.path.join(os.getcwd(), "brave_automation_profile")
...
async def extract_restaurant_urls(page):
    ...
    print(f"[*] Obiettivo: scansionare {'TUTTE le pagine' if pages_to_scrape == float('inf') else f'{pages_to_scrape} pagina/e'}.\n")
    ...
    while current_page <= pages_to_scrape:
        ...
        next_button = page.locator('a[data-smoke-attr="pagination-next-arrow"][aria-label="Pagina successiva"]')
        ...
        if next_button_count > 0:
            ...
            if href_estratto:
                ...
                print("   [✗] Attributo href del pulsante 'Avanti' non valido o assente. Fine delle pagine.")
                break
        else:
            print("   [✗] Pulsante 'Avanti' con gli attributi specificati non trovato. Siamo sull'ultima pagina.")
            break
...
async def extract_restaurant_features(page, url):
    ...
    data["restaurant_name"] = await safe_text(page.locator('div[data-test-target="restaurant-detail-info"] h1'))
    ...
    data["total_review"] = await safe_text(page.locator('div[data-automation="bubbleReviewCount"] span'))
    await micro_reading_pause(page)
    
    # === FEATURE 4: cuisine_type ===
    try:
        cuisine_locators = page.locator('div[data-test-target="restaurant-detail-info"] a[href*="/Restaurants-g187849-c"]')
    ...
    # === FEATURE 7: address ===
    data["address"] = await safe_text(page.locator('span[data-automation="restaurantsMapLinkOnName"]'))
    await micro_reading_pause(page)
    
    # === FEATURE 8: website ===
    website_href = await safe_attr(page.locator('a[data-automation="restaurantsWebsiteButton"]'), 'href')
            ...
            for idx, url in enumerate(urls_to_scrape, 1):
                ...
                try:
                    ...
                    if restaurant_data:
                        ...
                        print(f"   [!] Estrazione fallita per URL.")
                
                except Exception as e:
                    print(f"   [!] Errore critico durante lo scraping: {str(e)}")
                    mark_url_failed(checkpoint, url)
                    print(f"   [!] URL segnato come fallito nel checkpoint.")
                    continue
