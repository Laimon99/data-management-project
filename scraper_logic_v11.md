...
**The solution implemented in `tripadvisor_scraper_v11.py`** systematically avoids this fragility by anchoring all selectors exclusively to structural attributes that encode semantic meaning and are therefore stable across deployments:
...
context = await p.chromium.launch_persistent_context(
    user_data_dir=USER_DATA_DIR,   # Path: ./brave_automation_profile
...
URLs are written to `tripadvisor_list_restaurant.txt` in **write mode after each page** (rebuilding from the in-memory list), with deduplication enforced by the `if full_url not in extracted_urls` guard. The file therefore converges to a clean, unique set of restaurant URLs.
...
```
tripadvisor_list_restaurant.txt      ← Source of truth for all URLs
tripadvisor_scraper_results.json     ← Accumulated structured data output
tripadvisor_checkpoint.json          ← Progress tracking state
```
...
FAILURE EVENT
     │
     ▼
tripadvisor_checkpoint.json contains last known good state
     │
     ▼
...
Script resumes from the first unprocessed URL
     │
     ▼
tripadvisor_scraper_results.json loaded with prior records
     │
     ▼
new results appended to existing dataset
```

This architecture guarantees that the final `tripadvisor_scraper_results.json` is always a complete, deduplicated, and chronologically coherent dataset regardless of how many sessions were required to produce it.

---

*End of Technical Architecture Report — `tripadvisor_scraper_v11.py`*
