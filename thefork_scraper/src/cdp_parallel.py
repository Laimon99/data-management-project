...
def run_parallel_graphql_cdp(options: ParallelCDPOptions) -> int:
    ...
    if options.wait_for_manual_ready:
        input(
            "Open/check TheFork in each Brave window, solve cookies/captcha/proxy prompts if needed, "
            "then press Enter to start GraphQL workers..."
        )
...
def launch_brave_worker(worker: dict, options: ParallelCDPOptions) -> subprocess.Popen:
    ...
    logging.info("Launching Brave worker %s with %s on port %s.", worker["worker_number"], proxy_config.safe_label(), worker["port"])
