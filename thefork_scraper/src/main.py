...
from .cdp_parallel import ParallelCDPOptions, default_parallel_profile_root, run_parallel_graphql_cdp
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--pause-on-antibot",
        action="store_true",
        help="Pause for manual browser intervention when a detail page shows HTTP 403/429 or anti-bot markers.",
    )
    parser.add_argument(
        "--micro-pause-min-ms",
        type=int,
    ...
    parser.add_argument(
        ...
        type=float,
        default=10800.0,
        help="Cooldown applied to a proxy after a detail page returns HTTP 403 in round-robin mode.",
    )
    parser.add_argument(
        "--pool-403-threshold",
        type=int,
        default=3,
        help="Stop round-robin mode when this many distinct proxies return HTTP 403 inside the pool window.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Defaults --user-data-dir to output/brave_automation_profile."
        ),
    )
    parser.add_argument(
        "--browser-warmup-url",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Resume missing detail enrichment through TheFork GraphQL calls executed inside an already-open "
            "manual Brave/Chromium tab connected with --connect-over-cdp-url."
        ),
    )
    parser.add_argument(
        "--graphql-review-size",
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="Number of Brave proxy profiles / GraphQL workers to run in --graphql-cdp-parallel-proxies mode.",
    )
    parser.add_argument(
        "--parallel-base-port",
        type=int,
    ...
    parser.add_argument(
        ...
        default=None,
        help="Directory for parallel Brave user data dirs. Defaults to C:/tmp/thefork_cdp_proxy_profiles on Windows.",
    )
    parser.add_argument(
        "--parallel-output-root",
        default="output/runs/graphql_cdp_parallel",
        help=(
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Start workers immediately after CDP ports are ready instead of waiting for manual TheFork checks.",
    )
    parser.add_argument(
...
def resolve_input_partial_path(project_root: Path, raw_path: str | None) -> Path:
    ...
    return project_root / "output" / "thefork_milan_restaurants_normalized_partial.json"
