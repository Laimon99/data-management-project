...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Merge TheFork scraper JSON outputs, keeping the richest record for each restaurant."
    )
    ...
    parser.add_argument(
        "--report",
        default=None,
        help="Optional merge audit report JSON path.",
    )
...
def main() -> None:
    ...
    print(f"Merged {len(merged_records)} TheFork records into {output_path}")
...
def field_weight(key: str, value: Any) -> int:
    ...
    if key == "social_links" and isinstance(value, dict):
        return 2 + len(value)
    if key in {"website", "phone", "phone_number", "email", "opening_hours", "working_days_hours", "working_hours_structured", "latitude", "longitude"}:
        return 3
    return 1
