...
from .scraper import DEFAULT_BROWSER_CHANNELS, DEFAULT_USER_AGENT, START_URL, normalize_browser_channel
...
def run_detail_block_calibration(storage: JsonStorage, options: CalibrationOptions) -> dict[str, Any]:
    ...
    if completed_count and pending_records:
        report["notes"].append(
            "The current partial already contains completed detail records and remaining records, so it is evidence of an earlier block or interruption."
        )
        ...
        try:
            with sync_playwright() as playwright:
                launch_options: dict[str, Any] = {
                    ...
                    "args": [] if options.browser_executable_path else ["--disable-blink-features=AutomationControlled"],
                }
                if options.browser_executable_path:
...
def run_detail_attempts_with_existing_browser(
    ...
    if proxy is not None:
        logging.warning(
            "Ignoring proxy %s because --connect-over-cdp-url uses the already-open browser network.",
            proxy.safe_label(),
        )
    ...
    with sync_playwright() as playwright:
        try:
            ...
            raise RuntimeError(f"Could not connect to existing browser at {options.connect_over_cdp_url}: {error}") from error
...
def choose_recommended_strategy(report: dict[str, Any], test_runs: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "why": (
            "Use one browser session per proxy, rotate proxies between batches, and start parallel workers only after "
