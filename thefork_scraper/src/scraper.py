...
class TheForkScraper:
    ...
    def _scrape_with_channel(
        ...
        with sync_playwright() as playwright:
            launch_options: dict[str, Any] = {
                ...
                "args": [] if self.browser_executable_path else ["--disable-blink-features=AutomationControlled"],
            }
            if self.browser_executable_path:
    ...
    def _warm_up_browser_profile(self, page: Page) -> None:
        ...
        logging.info("Warming up browser profile on %s for %.1f seconds.", target_url, self.browser_warmup_seconds)
        try:
    ...
    def _proxy_round_robin_until_complete(
        ...
        if restored_deferrals:
            logging.info("Restored %s previously failed detail URLs to deferred state.", restored_deferrals)
        ...
        if self.proxy_configs and self.user_data_dir is None:
            logging.warning(
                ...
                "Use --user-data-dir output/browser_profile to isolate browser profiles per proxy."
            )
        ...
        if is_future_timestamp(proxy_state.get("global_blocked_until")):
            logging.warning(
                "The proxy pool is in global cooldown until %s. Stopping before making new requests.",
        ...
        for cycle in range(1, self.max_auto_detail_cycles + 1):
            ...
            started_at = time.monotonic()
            result = self._scrape_proxy_turn(channel, records, state["proxy"], record_limit, deferrals, cycle)
            records = result.pop("records")
            duration_seconds = round(time.monotonic() - started_at, 3)
            pending_after = count_pending_details(records, record_limit)
            ...
            if last_succeeded_url:
                ...
                state["last_success_at"] = proxy_state["proxies"][state["proxy_label"]].get("last_success_at")
                ...
                state["last_failure_at"] = proxy_state["proxies"][state["proxy_label"]].get("last_failure_at")
            ...
            if block_failure:
                ...
                if self.proxy_403_cooldown_seconds > 0:
                    ...
                    set_proxy_blocked_until(proxy_state, state["proxy_label"], state["blocked_until"])
                    logging.warning(
            ...
            block_events = http_403_failures or (
                [{"url": last_failed_url or last_attempted_url, "status": result.get("last_failure_http_status")}]
                if block_failure
                else []
            )
            ...
            if pool_block_triggered and self.stop_on_pool_block:
                logging.warning(
                    "Stopping round-robin mode because the proxy pool reached the HTTP 403 threshold. "
                    "Global cooldown until %s.",
                    proxy_state.get("global_blocked_until"),
                )
                return records
    ...
    def _next_round_robin_proxy_state(self, active_states: list[dict[str, Any]]) -> dict[str, Any]:
        return min(
            active_states,
            key=lambda state: (self._proxy_available_monotonic(state), state["turns"], state["proxy_index"]),
        )
            ...
            with sync_playwright() as playwright:
                ...
                try:
                    ...
                    while processed_in_turn < self.restaurants_per_proxy_turn:
                        next_record = self._next_round_robin_record(records, record_limit, deferrals, cycle)
...
def set_proxy_blocked_until(proxy_state: dict[str, Any], proxy_label: str, blocked_until: str | None) -> None:
...
def record_proxy_403_event(
    ...
    recent_events = prune_recent_403_events([*proxy_state.get("recent_403_events", []), event], window_seconds)
    proxy_state["recent_403_events"] = recent_events
    proxy_state["updated_at"] = utc_timestamp()

    distinct_proxy_labels = {
        item.get("proxy_label")
        for item in recent_events
        if item.get("proxy_label")
    }
...
def summarize_recent_403_events(proxy_state: dict[str, Any], limit: int = 30) -> list[dict[str, Any]]:
...
def summarize_deferrals(deferrals: dict[str, Any], limit: int = 30) -> list[dict[str, Any]]:
    items = sorted(
        deferrals.values(),
        key=lambda item: (int(item.get("deferred_until_cycle") or 0), -(int(item.get("failures") or 0))),
    )
    return [
        {
            "url": item.get("url"),
