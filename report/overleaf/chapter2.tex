...
\textbf{Size} &
\textbf{Format} &
\begin{tabular}[t]{@{}l@{}}\textbf{Natural}\\\textbf{Key}\end{tabular} &
\begin{tabular}[t]{@{}l@{}}\textbf{Destination}\\\textbf{Collection}\end{tabular} \\
\midrule
...
249 MB &
JSON Lines &
{\scriptsize\texttt{place\_id}} &
{\scriptsize\texttt{restaurants\_raw\_google}} \\

Tripadvisor &
7,539 &
49 MB &
JSON Array &
{\scriptsize\texttt{source\_url}} &
{\scriptsize\texttt{restaurants\_raw\_tripadvisor}} \\

TheFork &
1,344 &
11.1 MB &
JSON Array &
{\scriptsize\texttt{source\_id}} &
...
\label{sec:pre-integration-quality}
...
The assessment follows standard data-quality dimensions~\cite{batini-scannapieco-2016}, and the metrics were computed according to the following definitions:
...
\item \textbf{Validity} (format-level check: a present value is valid if it matches its expected source-specific format)
\[
V =
100 \times
...
S_{\mathrm{spatial}} =
100 \times
\frac{\text{Valid-coordinate records}}
{\text{Total records}}
\]
...
\[
\begin{aligned}
T_{\mathrm{time}} &= \max(0,\;100 \times (1 - r))\\
r &= \frac{\text{Collection duration}}{\text{Refresh target}}
\end{aligned}
\]
...
\begin{aligned}
Score =\;&
0.25 C_{\mathrm{crit}} + 0.20 V + 0.15 S_{\mathrm{spatial}}\\
&+ 0.15 U + 0.10 T_{\mathrm{time}} + 0.15 R
\end{aligned}
\]
...
Spatial Readiness & 100.00\% & 0.00\% & 100.00\% \\

Timeliness & 67.23\% & 43.75\% & 95.83\% \\

Reliability & 79.50\% & 53.10\% & 88.84\% \\

Flags / 100 Records & 27.17 & 48.04 & 17.04 \\
...
Only 229 quality flags were generated (17 per 100 records), representing the lowest anomaly density among all sources. Consequently, TheFork can already be considered highly suitable for integration and geospatial matching, requiring only limited preprocessing before the subsequent stages of the pipeline. The complete field-level coverage tables and anomaly profile are included in Appendix~\ref{app:pre-integration-quality}.
