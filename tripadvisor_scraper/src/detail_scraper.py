...
PRICE_RANGE_PATTERN = re.compile(r"(\${1,4}(?:\s*-\s*\${1,4})?|\u20ac{1,4}(?:\s*-\s*\u20ac{1,4})?)")
REVIEW_COUNT_TEXT_PATTERN = re.compile(r"([\d.,\s]+)\s+(?:recensioni|reviews?)", re.IGNORECASE)
DISCOUNT_TEXT_PATTERN = re.compile(r"\b(?:discount|off|scont[io]|offer)\b[^\n]{0,80}\d+\s*%", re.IGNORECASE)
PHOTO_COUNT_PATTERN = re.compile(r"([\d.,\s]+)\s+(?:foto|photos?)", re.IGNORECASE)


class TripadvisorDetailScraper:
    def __init__(
        self,
    ...
    def enrich_record(
        ...
        scraped_at: str,
        blocked_detector: Callable[[Page], bool] | None = None,
        unlock_callback: Callable[[Page], None] | None = None,
    ) -> RestaurantRecord:
        ...
        try:
            status = self._load_detail_page(page, record.restaurant_url)
            if status == 403 or (blocked_detector and blocked_detector(page)):
                if unlock_callback:
                    unlock_callback(page)
                    status = self._load_detail_page(page, record.restaurant_url)
                else:
                    logging.warning("Tripadvisor detail page was blocked: %s", record.restaurant_url)
                    record.detail_scraped = False
                    record.scraped_at = scraped_at
                    return record
    ...
    def _load_detail_page(self, page: Page, url: str) -> int | None:
        ...
        return page.evaluate(
            ...
            (maxScriptChars) => {
              ...
              return {
                ...
                body_text: (body.innerText || '').slice(0, 140000),
    ...
    def _merge_detail_data(
        ...
        record.restaurant_name = normalize_name_candidate(record.restaurant_name) or record.restaurant_name
...
def extract_structured_restaurant_data(json_documents: list[Any]) -> dict[str, Any]:
    ...
    return {
        "restaurant_name": normalize_name_candidate(clean_optional(restaurant_node.get("name"))),
...
def extract_embedded_data(json_documents: list[Any], body_text: str, links: list[dict[str, str]]) -> dict[str, Any]:
    ...
    cuisine_type, price_range = split_cuisine_price(
        clean_optional(first_json_value(nodes, {"cuisine", "servesCuisine", "category", "categories"}))
    )

    return {
        "restaurant_name": normalize_name_candidate(clean_optional(first_json_value(nodes, {"restaurantName", "name"}))),
...
def extract_visible_data(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    cuisine_type, price_range = extract_visible_cuisine_and_price(lines)

    return {
        "restaurant_name": normalize_name_candidate(first_value(payload.get("h1"), payload.get("og_title"))),
...
def iter_json_nodes(value: Any) -> Iterable[dict[str, Any]]:
    stack = [value]
    visited = 0
    while stack and visited < 80_000:
        visited += 1
        item = stack.pop()
...
def split_cuisine_price(value: str | None) -> tuple[str | None, str | None]:
    if not value:
        return None, None
    match = PRICE_RANGE_PATTERN.search(value)
    if not match:
        return clean_optional(value), None
    price_range = clean_spaces(match.group(1))
    cuisine_type = clean_optional(value[: match.start()].strip(" ,-"))
    return cuisine_type, price_range
...
def normalize_external_url(value: Any) -> str | None:
    ...
    if TRIPADVISOR_HOST in host or "tripadvisor" in host:
        return None
    excluded_hosts = ("google.", "facebook.", "instagram.", "youtube.", "tiktok.", "doubleclick.", "datadome.")
...
def extract_address_from_lines(lines: list[str]) -> str | None:
    ...
    for line in lines:
        if street_pattern.search(line) and len(line) <= 160:
            return line
    return None
...
def extract_visible_rating(lines: list[str]) -> float | None:
    for line in lines:
        match = re.search(r"\b([0-5][,.][0-9])\b", line)
        if match and "review" not in line.lower() and "recension" not in line.lower():
...
def extract_visible_cuisine_and_price(lines: list[str]) -> tuple[str | None, str | None]:
    for line in lines:
        lowered = line.lower()
        if len(line) > 120 or any(token in lowered for token in ("review", "recension", "foto", "photo", "prenota", "book")):
            continue
        cuisine_type, price_range = split_cuisine_price(line)
        if cuisine_type or price_range:
            return cuisine_type, price_range
    return None, None
...
def extract_visible_photo_count(body_text: str, images: list[dict[str, str]]) -> int | None:
    match = PHOTO_COUNT_PATTERN.search(body_text or "")
    if match:
        return parse_int(match.group(1))
...
def is_restaurant_image_url(url: str, alt: str) -> bool:
    ...
    if not url.startswith(("http://", "https://")):
        return False
    excluded = ("logo", "icon", "avatar", "pixel", "sprite", "badge", "tracking", "member")
...
def extract_review_snippets_from_text(body_text: str) -> list[str]:
    ...
    for line in split_visible_lines(body_text):
        if re.search(r"\b(?:leggi le recensioni|read reviews|questo ristorante|this restaurant|does this restaurant)\b", line, re.IGNORECASE):
            continue
        if re.search(r"\b(?:sconto|discount|off|price|prezzo)\b", line, re.IGNORECASE):
            continue
        if 45 <= len(line) <= 280 and re.search(r"\b(?:excellent|ottim|servizio|cibo|food|locale|restaurant|ristorante)\b", line, re.IGNORECASE):
...
def extract_reviews_from_nodes(nodes: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for node in nodes:
        ...
        if {"reviewbody", "publishedtext"} & keys or ("text" in keys and ("rating" in keys or "author" in keys)):
