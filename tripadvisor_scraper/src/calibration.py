...
def run_detail_block_calibration(storage: JsonStorage, options: CalibrationOptions) -> dict[str, Any]:
    options.output_dir.mkdir(parents=True, exist_ok=True)
    records = load_source_records(storage)
    pending_records = [record for record in records if record.restaurant_url and not record.detail_scraped]
    ...
    report: dict[str, Any] = {
        ...
        "proxy_state": {
            ...
            "proxy_tests_status": "configured" if options.proxy_configs else "not_run_no_proxy_list",
            "configured_proxy_labels": [proxy.safe_label() for proxy in options.proxy_configs],
        },
        "manual_unlock": {
            "enabled": options.manual_unlock,
            "user_data_dir": str(options.user_data_dir) if options.user_data_dir else None,
            "unlock_timeout_seconds": options.unlock_timeout_seconds,
        },
        "test_plan": {
    ...
    if options.manual_unlock:
        report["notes"].append(
            "Manual unlock is enabled. If DataDome appears, the calibration waits for the user and then continues automatically."
        )
    ...
    if options.max_records <= 0:
        ...
        for delay_seconds in options.delay_values:
            ...
            for proxy in build_proxy_test_sequence(options.proxy_configs, options.max_proxies):
                records_for_run = selected_records[record_offset : record_offset + options.batch_size]
    ...
    historical_test_runs = load_historical_test_runs(options.output_dir, "tripadvisor")
    ...
    save_report(options.output_dir / "tripadvisor_block_calibration_report.json", report)
    return report
...
def build_proxy_test_sequence(proxy_configs: list[ProxyConfig], max_proxies: int) -> list[ProxyConfig | None]:
...
def run_single_detail_calibration(
    ...
    try:
        ...
        logging.warning("Tripadvisor calibration run failed: %s", error)

    result["duration_seconds"] = round(time.monotonic() - started_monotonic, 3)
    if result["succeeded"]:
        result["average_seconds_per_success"] = round(result["duration_seconds"] / result["succeeded"], 3)
    return result, sample_records
...
def run_detail_attempts(
    ...
    channels = [normalize_browser_channel(options.browser_channel)] if options.browser_channel else list(DEFAULT_BROWSER_CHANNELS)
    ...
    for channel in channels:
        try:
            with sync_playwright() as playwright:
                ...
                if options.manual_unlock and options.headless:
                ...
                if options.user_data_dir is not None:
                    ...
                    logging.info("Using Tripadvisor calibration profile: %s", profile_dir)
                ...
                try:
                    execute_record_attempts(detail_scraper, page, sample_records, delay_seconds, options, result)
            ...
            last_error = error
            logging.warning("Could not run calibration with channel %s: %s", channel or "bundled chromium", error)

    if last_error is not None:
        raise RuntimeError(f"No browser channel could run the calibration: {last_error}") from last_error
...
def execute_record_attempts(
    ...
    scraped_at = utc_now()
    unlock_callback = (lambda current_page: wait_for_manual_unlock(current_page, options)) if options.manual_unlock else None

    for index, record in enumerate(sample_records, start=1):
        logging.info(
            "Tripadvisor calibration %s: detail attempt %s/%s",
            result["run_id"],
            index,
            len(sample_records),
        )
        was_blocked_before = is_blocked_page(page)
        enriched_record = detail_scraper.enrich_record(
            page,
            record,
            scraped_at,
            blocked_detector=is_blocked_page,
            unlock_callback=unlock_callback,
        )
        was_blocked_after = is_blocked_page(page)
        ...
        if enriched_record.detail_scraped:
            ...
            if was_blocked_before or was_blocked_after:
                result["blocked_or_captcha_failures"] += 1
...
def wait_for_manual_unlock(page: Page, options: CalibrationOptions) -> None:
    logging.warning(
        "Tripadvisor calibration detected DataDome or captcha. Solve it in the browser window; calibration will continue automatically."
    )
    deadline = time.monotonic() + options.unlock_timeout_seconds
    while time.monotonic() < deadline:
        if not is_blocked_page(page):
            logging.info("Manual unlock completed for calibration.")
            return
        try:
            page.wait_for_timeout(1_000)
        except PlaywrightError:
            time.sleep(1)
    raise RuntimeError("Manual unlock timed out during calibration.")
...
def estimate_proxy_completion(
    ...
    fallback_seconds = max(delay_values or [2.0]) + 25.0
    seconds_per_detail = (
    ...
    for proxy_count in range(1, max(1, max_proxies) + 1):
        ...
        cooldown_cycles = max(0, int((records_per_proxy - 1) // safe_batch_size)) if safe_batch_size else 0
...
def choose_recommended_strategy(report: dict[str, Any], test_runs: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    margin_feasible = [estimate for estimate in report["estimates"] if estimate["estimated_hours"] <= margin_target_hours]
    recommended_proxy_count = (
        margin_feasible[0]["proxy_count"]
        if margin_feasible
        else minimum_proxy_count
    )
    safe_batch_size = report["estimates"][0].get("safe_batch_size_used") if report["estimates"] else report["test_plan"]["batch_size"]
    best_run = best_successful_run(test_runs)
    delay_seconds = best_run["delay_seconds"] if best_run else max(report["test_plan"]["delay_values_seconds"] or [10.0])
    ...
    return {
        ...
        "rotation_mode": "isolated_profile_per_proxy_and_alternating_batches",
        "parallelism_mode": "staggered_parallel_after_manual_unlock_per_profile",
        "why": (
            "Tripadvisor protection is session-sensitive, so each proxy should use its own persistent profile. "
            "Run small calibrated batches first, then use staggered workers only for proxies that pass."
        ),
        "proxy_gap": (
            "No real proxy list was supplied, so direct-IP data and estimates are saved but proxy validation is still pending."
...
def save_sample_records(output_dir: Path, run_result: dict[str, Any], records: list[RestaurantRecord]) -> Path | None:
    if not records:
        return None
    ...
    with path.open("w", encoding="utf-8") as output_file:
        json.dump([record.to_dict() for record in records], output_file, ensure_ascii=False, indent=2)
        output_file.write("\n")
    return path
...
def save_report(path: Path, report: dict[str, Any]) -> None:
    ...
    with archive_path.open("w", encoding="utf-8") as output_file:
        json.dump(report, output_file, ensure_ascii=False, indent=2)
        output_file.write("\n")
    logging.info("Saved Tripadvisor calibration report to %s", path)


def build_run_id(delay_seconds: float, proxy: ProxyConfig | None) -> str:
    delay_label = str(delay_seconds).replace(".", "_")
    proxy_label = proxy.safe_label() if proxy else "direct_ip"
    return f"tripadvisor_delay_{delay_label}_{proxy_label}"
