...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Merge TheFork scraper JSON outputs, keeping the richest record for each restaurant."
        )
    )
...
def main() -> None:
    ...
    report_path = (
        Path(args.report) if args.report else output_path.with_suffix(".merge_report.json")
    )
