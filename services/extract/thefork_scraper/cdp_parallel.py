...
def run_parallel_graphql_cdp(options: ParallelCDPOptions) -> int:
    ...
    for worker in workers:
        browser_processes.append(launch_brave_worker(worker, options))
        wait_for_cdp(worker["port"])

    print_plan(workers, options)
    if options.prepare_only:
        logging.info(
            "Prepare-only mode finished after launching %s browser profiles.", len(workers)
        )
        return 0

    if options.wait_for_manual_ready:
        input(
            "Open/check TheFork in each Brave window, solve cookies/captcha/proxy prompts "
            "if needed, then press Enter to start GraphQL workers..."
        )

    worker_processes = []
    for worker in workers:
        worker_processes.append(subprocess.Popen(worker["command"], cwd=options.project_root))
    ...
    if failed_codes:
        logging.warning("One or more GraphQL/CDP workers failed: %s", failed_codes)
    ...
    if merge_report is not None:
        ...
        if pending_detail > 0:
            logging.warning(
                "Parallel GraphQL/CDP run finished, but %s records still miss detail data.",
                pending_detail,
            )
        else:
            logging.info("Parallel GraphQL/CDP run completed and all records have detail data.")
    else:
        logging.info("Parallel GraphQL/CDP worker processes completed.")
    return 0


def merge_parallel_worker_outputs(
    workers: list[dict], options: ParallelCDPOptions, run_started_at: float
) -> dict:
    ...
    final_output_path, pending_detail, final_output_written = write_final_output_if_complete(
        input_path, merged_records
    )
    ...
    if final_output_written:
        ...
        logging.warning(
            "Enriched final output not written because %s records still miss detail data.",
            pending_detail,
        )
...
def write_final_output_if_complete(input_path: Path, records: list[dict]) -> tuple[Path, int, bool]:
    final_output_path = input_path.parent / "thefork_milan_restaurants_enriched.json"
    pending_detail = count_pending_detail(records)
    if pending_detail == 0:
...
def count_pending_detail(records: list[dict]) -> int:
    detail_count = sum(1 for record in records if record.get("detail_scraped") is True)
    return len(records) - detail_count
...
def fresh_worker_partial_paths(workers: list[dict], run_started_at: float) -> list[Path]:
    ...
    for worker in workers:
        partial_path = worker["output_dir"] / "thefork_milan_restaurants_normalized_partial.json"
...
def backup_partial_path(input_path: Path) -> Path:
    ...
    return input_path.with_name(
        f"{input_path.stem}_before_parallel_merge_{timestamp}{input_path.suffix}"
    )
...
def build_worker_specs(options: ParallelCDPOptions) -> list[dict]:
    ...
    if (
        options.distributed_slot_count is not None
        and options.distributed_slot_start + worker_count > shard_count
    ):
        raise RuntimeError(
    ...
    for zero_index, proxy_config in enumerate(options.proxy_configs[:worker_count]):
        ...
        profile_dir = (
            options.profile_root / f"worker_{worker_number:02d}_{proxy_config.safe_label()}"
        )
...
def create_proxy_auth_extension(extension_dir: Path, proxy_config: ProxyConfig) -> None:
    ...
    if not parsed.hostname or not parsed.port:
        raise RuntimeError(
            f"Invalid proxy server for {proxy_config.safe_label()}: {proxy_config.server}"
        )
    ...
    manifest = {
        "manifest_version": 3,
        "name": f"TheFork proxy profile {proxy_config.safe_label()}",
        "version": "1.0.0",
        "permissions": [
            "proxy",
            "storage",
...
def default_parallel_profile_root(project_root: Path) -> Path:
    if os.name == "nt":
        return Path("C:/tmp/thefork_cdp_proxy_profiles")
    return project_root / "data/raw/thefork" / "cdp_proxy_profiles"
