...
def run_graphql_detail_enrichment(
    storage: JsonStorage, options: GraphQLDetailOptions
) -> dict[str, Any]:
    records = storage.load_partial()
    if not records:
        raise RuntimeError(
            "No partial records found. Run listing scraping or pass --input-partial first."
        )
    ...
    logging.info(
        "Starting TheFork GraphQL/CDP detail enrichment for %s pending records.",
        len(pending_indexes),
    )
        ...
        try:
            ...
            for record_index in pending_indexes:
                ...
                try:
                    ...
                    report["failed_urls"].append(record.restaurant_url)
                    logging.warning(
                        "GraphQL detail enrichment failed for %s: %s", record.restaurant_url, error
                    )
                    if consecutive_failures >= options.max_consecutive_failures:
                        ...
                        logging.warning(
                            "Stopping GraphQL enrichment after %s consecutive failures.",
...
def merge_graphql_detail(
    record: RestaurantRecord, payload: dict[str, Any], scraped_at: str
) -> None:
    ...
    if reviews:
        ...
        record.review_snippets = unique_texts(
            [*record.review_snippets, restaurant["reviewsSummary"]["content"]]
        )
...
def parse_money(value: dict[str, Any] | None) -> str | None:
    ...
    amount_text = (
        str(int(amount)) if amount.is_integer() else f"{amount:.2f}".rstrip("0").rstrip(".")
    )
...
def count_photos(restaurant: dict[str, Any]) -> int | None:
    ...
    photo_ids = {
        clean_optional(photo.get("id"))
        or clean_optional(photo.get("src"))
        or clean_optional(photo.get("imageUrl"))
...
def save_graphql_report(
    storage: JsonStorage, options: GraphQLDetailOptions, report: dict[str, Any]
) -> None:
